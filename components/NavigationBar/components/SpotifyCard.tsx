...
const SpotifyCard: React.FC<SpotifyCardProps> = ({ isPlaying, imgLink, artistName, trackName, trackLink }) => {
            ...
            <button onClick={handleOpenLink} className="flex w-80 h-full bg-[var(--bg-base)] border border-[var(--bg-border)] rounded-[20px] z-20 shadow-lg p-3 gap-4 focus:outline-none active:scale-90 transition duration-150 ease-out overflow-hidden">
                ...
                <div className={`flex flex-1 flex-col ${isPlaying ? "gap-4" : "gap-8"}`}>
                    ...
                    <div className="flex flex-1 flex-col gap-1">
                        <div className="text-xs font-normal text-start text-[var(--label-muted)] max-w-[200px] overflow-hidden text-ellipsis whitespace-nowrap">{artistName}</div>
                        <div className="flex gap-1 items-end">
                            ...
                            <div className={`flex h-6 w-6 items-center justify-center text-[var(--label-muted)] rounded-lg ${isPlaying ? "" : "hidden"}`}>
